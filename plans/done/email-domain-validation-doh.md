...
document.getElementById(prefix + "-form").addEventListener("submit", async function(e) {
  ...
  fetch('https://formspree.io/f/mrejlned', {
    ...
    if (res.ok) {
      ...
      if (fsError) { fsError.classList.remove('cv-error-hidden'); fsError.textContent = locale.t('errSendFailed'); }
    }
  }).catch(function() {
    if (submitBtn) submitBtn.disabled = false;
    var fsError = document.querySelector('#' + prefix + '-modal [data-fs-error]');
  ...
  fetch(BOOKING_SCRIPT_URL + '?' + params.toString())
    ...
    .then(function(data) {
      if (data.success) {
        ...
        document.getElementById(p + '-bk-confirm-detail').textContent =
          formatSlot(new Date(selectedSlot.start), new Date(selectedSlot.end));
...
```html
<input type="text" id="PREFIX-bk-hp" name="bk-hp" style="display:none" tabindex="-1" autocomplete="off">
```
