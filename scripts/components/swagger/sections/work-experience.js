...
export function renderWorkExperienceSection(data) {
  ...
  data.workExperience.forEach(function (exp, i) {
    ...
    var ep;
    if (methods[i] === 'POST') {
      ep = swgPost('workExperience', id, path, desc, descHtml, paramRows, [{code: '200', bodyHtml: respBody, linksHtml: linksHtml}]);
    } else {
      ep = swgPut('workExperience', id, path, desc, descHtml, paramRows, [{code: '200', bodyHtml: respBody, linksHtml: linksHtml}]);
    }
