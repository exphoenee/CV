...
export function renderSkillsSection(data) {
  ...
  var primarySkills = data.programmingLanguages.filter(function (p) {
    return ['TypeScript', 'JavaScript', 'CSS', 'SCSS', 'HTML'].indexOf(p.name) > -1;
  });
  var primaryResp = '';
  primarySkills.forEach(function (p) {
    primaryResp += '<p class="cv-kv"><code>' + E(p.name.toLowerCase()) + '</code>experto</p>';
  });
  primaryResp +=
  ...
  var backendSkills = data.programmingLanguages.filter(function (p) {
    return ['Python', 'PHP'].indexOf(p.name) > -1;
  });
  var backendResp = '';
  backendSkills.forEach(function (p) {
    backendResp += '<p class="cv-kv"><code>' + E(p.name.toLowerCase()) + '</code>proficient</p>';
  });
  backendResp +=
  ...
  var patchBlock =
    '<div class="opblock opblock-patch" id="operations-skills-patch_skills_testing"><div class="opblock-summary opblock-summary-patch"><button class="opblock-summary-control"><span class="opblock-summary-method">PATCH</span><div class="opblock-summary-path-description-wrapper"><span class="opblock-summary-path"><a class="nostyle"><span>/skills/testing</span></a></span><div class="opblock-summary-description">Testing & quality (improving daily)</div></div></button><div class="view-line-link copy-to-clipboard" title="Copy to clipboard">' +
    svgClipboard +
    ...
    svgArrowUp +
    '</button></div><div class="opblock-body">' +
    swgDesc('<p>Testing stack and methodologies. Coverage is improving daily.</p>') +
    ...
    swgParams(null) +
    '</div>' +
    swgResponses([{ code: '200', bodyHtml: testingBody, linksHtml: 'No links' }]) +
    '</div></div>';
  ...
  skillEndpoints.push(
    swgGet(
      ...
      null,
      [{ code: '200', bodyHtml: toolingResp, linksHtml: 'No links' }],
    ),
  );
  ...
  skillEndpoints.push(
    swgGet(
      ...
      null,
      [{ code: '200', bodyHtml: aiResp, linksHtml: 'No links' }],
    ),
  );
