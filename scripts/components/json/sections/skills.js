...
export function renderSkills(data, push) {
  ...
  entries.forEach(function ([key, { list, comment }], idx) {
    ...
    var items = list.map(s => html`<span class="s">"${s}"</span>`).join('<span class="p">, </span>');
    var line = html`<span class="k">"${key}"</span><span class="p">: [</span>${raw(items)}<span class="p">]${isLast ? '' : ','}</span>`;
  ...
  if (data.skillNote) {
    ...
    var noteLine = html`<span class="k">"${n.key}"</span><span class="p">: </span><span class="s">"${n.value}"</span>`;
