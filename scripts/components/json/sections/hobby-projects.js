...
export function renderHobbyProjects(data, push) {
  ...
  data.hobbyProjects.forEach(function (proj, hi) {
    ...
    var link = html`<a href="${proj.url}" target="_blank">${proj.name}</a>`;
    var line = html`<span class="p">{ </span><span class="k">"name"</span><span class="p">: </span><span class="s">"${proj.name}"</span><span class="p">, </span><span class="k">"url"</span><span class="p">: </span><span class="s">"${raw(link)}"</span><span class="p"> }</span>`;
