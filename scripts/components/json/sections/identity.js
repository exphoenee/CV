...
export function renderIdentity(data, push) {
  ...
  contactKeys.forEach(function (k, ki) {
    ...
    if (c && c.url) {
      ...
      var h = html`<span class="k">"${k}"</span><span class="p">: </span
        ><span class="s">"${raw(link)}"</span>`;
  ...
  data.identity.languages.forEach(function (lang, li) {
    ...
    var h = html`<span class="k">"${lang.name}"</span><span class="p">: </span
      ><span class="s">"${lang.level.toLowerCase()}"</span>`;
    if (comma) h += '<span class="p">,</span>';
