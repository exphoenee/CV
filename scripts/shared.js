...
export function skillChip(name, iconFile) {
  return (
    '<div class="skill"><span class="skillImage"><img src="./assets/images/' +
    iconFile +
    '" alt="' +
    escHtml(name) +
    '" title="' +
    escHtml(name) +
    '" onerror="this.style.visibility=\'hidden\'" /></span><span>' +
    escHtml(name) +
    '</span></div>'
  );
}
      ...
      .then(function (res) {
        if (res.ok) {
          ...
          if (fsError) {
            ...
            fsError.textContent = locale.t('errSendFailed') || 'Failed to send. Please try again.';
          }
        }
      })
      .catch(function () {
...
function musicGenreOptionsHTML() {
  return MUSIC_GENRES.map(function (g) {
    return (
      '            <div class="custom-option" data-value="' + g.value + '">' + g.label + '</div>'
    );
  }).join('\n');
}
...
export function musicPlayerHTML() {
  ...
  return (
    ...
    '  <button id="music-toggle" aria-label="' +
    t('ariaOpenMusicPlayer') +
    '" title="Music Player"><i class="fas fa-music" aria-hidden="true"></i></button>' +
    '  <div id="music-player-box" class="music-box-hidden" role="dialog" aria-modal="false" aria-label="Music player controls">' +
    '    <button id="music-box-close" class="music-box-close-btn" aria-label="' +
    t('ariaCloseMusicPlayer') +
    '" title="Close">\u2715</button>' +
    ...
    '          <input type="range" id="track-seek" min="0" max="100" value="0" step="0.1" class="track-seek-slider" aria-label="' +
    t('ariaTrackPosition') +
    '" />' +
    '          <span class="track-time-label" id="track-time-total" aria-label="Total duration">00:00</span>' +
    '        </div>' +
    '        <div class="custom-select" id="custom-genre-select" role="combobox" aria-label="' +
    t('ariaGenreSelect') +
    '" aria-haspopup="listbox" aria-expanded="false">' +
    '          <div class="custom-select-trigger" role="button" tabindex="0" aria-label="' +
    t('ariaGenreSelect') +
    '"><span>' +
    MUSIC_GENRES[0].label +
    ...
    '          <button id="music-repeat" class="transport-btn" title="Repeat mode" aria-label="' +
    t('ariaToggleRepeat') +
    '" aria-pressed="false"><i class="fas fa-repeat" aria-hidden="true"></i></button>' +
    '          <button id="music-prev" class="transport-btn" title="Previous" aria-label="' +
    t('ariaPrevTrack') +
    '"><i class="fas fa-backward-step" aria-hidden="true"></i></button>' +
    '          <button id="music-playpause" class="transport-btn" title="Play/Pause" aria-label="' +
    t('ariaPlayMusic') +
    ...
    '          <input type="range" id="music-volume" min="0" max="1" step="0.05" value="0.5" class="volume-slider" aria-label="' +
    t('ariaMusicVolume') +
    '" />' +
    '        </div>' +
...
export function bookingModalHTML(prefix) {
  ...
  return [
    '<div id="' +
      p +
      '-booking-modal" class="cv-modal-hidden" role="dialog" aria-modal="true" aria-labelledby="' +
      p +
      '-bk-title">',
    ...
    '      <button class="hire-close" id="' +
      p +
      '-bk-close" type="button" aria-label="' +
      t('ariaCloseBooking') +
      '">&#x2715;</button>',
    '    </div>',
    '    <div class="bk-body">',
    ...
    '      <div id="' +
      p +
      '-bk-step-date" class="bk-step bk-hidden" role="region" aria-label="Step 1: Choose a date">',
    '        <p class="bk-step-label" data-bk-i18n="bookStep1">' + t('bookStep1') + '</p>',
    '        <div id="' +
      p +
      '-bk-dates" class="bk-dates-grid" role="list" aria-label="Available dates"></div>',
    '      </div>',

    '      <div id="' +
      p +
      '-bk-step-time" class="bk-step bk-hidden" role="region" aria-label="Step 2: Choose a time slot">',
    '        <button class="bk-back-btn" id="' +
      p +
      '-bk-back-date" aria-label="Back to date selection"><i class="fa-solid fa-arrow-left" aria-hidden="true"></i> <span data-bk-i18n="bookBack">' +
      t('bookBack') +
      '</span></button>',
    ...
    '        <div id="' +
      p +
      '-bk-slots" class="bk-slots-grid" role="list" aria-label="Available time slots"></div>',
    '      </div>',

    '      <div id="' +
      p +
      '-bk-step-form" class="bk-step bk-hidden" role="region" aria-label="Step 3: Your details">',
    '        <button class="bk-back-btn" id="' +
      p +
      '-bk-back-time" aria-label="Back to time slot selection"><i class="fa-solid fa-arrow-left" aria-hidden="true"></i> <span data-bk-i18n="bookBack">' +
      t('bookBack') +
      '</span></button>',
    ...
    '          <input type="text" id="' +
      p +
      '-bk-hp" name="bk-hp" style="display:none" tabindex="-1" autocomplete="off">',
    '          <div class="bk-field">',
    '            <label for="' +
      p +
      '-bk-name"><span data-bk-i18n="bookYourName">' +
      t('bookYourName') +
      '</span> <span class="bk-required" aria-label="required">*</span></label>',
    '            <input type="text" id="' +
      p +
      '-bk-name" name="bk-name" required placeholder="Jane Smith" aria-required="true" autocomplete="name">',
    '          </div>',
    '          <div class="bk-field">',
    '            <label for="' +
      p +
      '-bk-email"><span data-bk-i18n="bookYourEmail">' +
      t('bookYourEmail') +
      '</span> <span class="bk-required" aria-label="required">*</span></label>',
    '            <input type="email" id="' +
      p +
      '-bk-email" name="bk-email" required placeholder="your@email.com" aria-required="true" aria-describedby="' +
      p +
      '-bk-email-err" autocomplete="email">',
    '            <span class="bk-field-error" id="' +
      p +
      '-bk-email-err" role="alert" aria-live="polite"></span>',
    '          </div>',
    '          <div class="bk-field">',
    '            <label for="' +
      p +
      '-bk-topic"><span data-bk-i18n="bookTopic">' +
      t('bookTopic') +
      '</span> <span class="bk-required" aria-label="required">*</span></label>',
    '            <textarea id="' +
      p +
    ...
    '        <button class="bk-btn-secondary" id="' +
      p +
      '-bk-new" data-bk-i18n="bookNewBooking">' +
      t('bookNewBooking') +
      '</button>',
    '      </div>',
    ...
    '      <div id="' +
      p +
      '-bk-cooldown" class="bk-state bk-hidden" role="status" aria-live="polite">',
    '        <div class="bk-state-icon" aria-hidden="true"><i class="fa-regular fa-calendar-check"></i></div>',
    '        <p class="bk-confirm-title" data-bk-i18n="bookCooldownTitle">' +
      t('bookCooldownTitle') +
      '</p>',
    '        <p class="bk-confirm-note" data-bk-i18n="bookCooldownNote">' +
      t('bookCooldownNote') +
      '</p>',
    '      </div>',
...
export function initBookingModal(prefix) {
  ...
  function renderDates() {
    ...
    Object.keys(grouped).forEach(function (dateKey) {
      ...
      btn.setAttribute(
        'aria-label',
        formatDay(date) + ', ' + formatDate(date) + ' — ' + n + ' ' + slotWord,
      );
      btn.innerHTML =
        '<span class="bk-date-day">' +
        formatDay(date) +
        '</span>' +
        '<span class="bk-date-date">' +
        formatDate(date) +
        '</span>' +
        '<span class="bk-date-count" aria-hidden="true">' +
        n +
        ' ' +
        slotWord +
        '</span>';
      btn.addEventListener('click', function () {
        document.getElementById(p + '-bk-date-badge').textContent =
  ...
  document.getElementById(p + '-bk-back-date').addEventListener('click', function () {
    show(p + '-bk-step-date');
  });
...
export function hireModalHTML(prefix, opts) {
  ...
  return [
    ...
    '<div id="' +
      prefix +
      '-modal" class="cv-modal-hidden" role="dialog" aria-modal="true" aria-labelledby="' +
      prefix +
      '-dialog-title">',
    ...
    '      <h3 id="' +
      prefix +
      '-dialog-title" data-hire-i18n="contactTitle">' +
      t('contactTitle') +
      '</h3>',
    '      <button class="hire-close" id="' +
      prefix +
      '-close" type="button" aria-label="' +
      t('ariaCloseContactForm') +
      '">&#x2715;</button>',
    '    </div>',
    ...
    '        <label for="' +
      prefix +
      '-name" data-hire-i18n="yourName">' +
      t('yourName') +
      '</label>',
    '        <input id="' +
      prefix +
      '-name" type="text" name="name" required placeholder="' +
      t('namePlaceholder') +
      '" data-hire-i18n-placeholder="namePlaceholder" data-fs-field aria-required="true" aria-describedby="' +
      prefix +
      '-name-err" autocomplete="name" />',
    '        <span data-fs-error="name"' + errClass + '></span>',
    '        <span class="hire-field-error" id="' +
      prefix +
      '-name-err" role="alert" aria-live="polite"></span>',
    '      </div>',
    '      <div class="hire-field">',
    '        <label for="' +
      prefix +
      '-email" data-hire-i18n="yourEmail">' +
      t('yourEmail') +
      '</label>',
    '        <input id="' +
      prefix +
      '-email" type="email" name="email" required placeholder="' +
      t('emailPlaceholder') +
      '" data-hire-i18n-placeholder="emailPlaceholder" data-fs-field aria-required="true" aria-describedby="' +
      prefix +
      '-email-err" autocomplete="email" />',
    '        <span data-fs-error="email"' + errClass + '></span>',
    '        <span class="hire-field-error" id="' +
      prefix +
      '-email-err" role="alert" aria-live="polite"></span>',
    '      </div>',
    '      <div class="hire-field">',
    '        <label for="' +
      prefix +
      '-message" data-hire-i18n="message">' +
      t('message') +
      '</label>',
    '        <textarea id="' +
      prefix +
