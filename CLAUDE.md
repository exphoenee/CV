...
2. **`cv-versions/history.md`** — append-only audit log, one row per CV event. **Mutations** (job-apply, cv-improver, cv-restore), **backups**, and read-only **reviews** (hr-review, language-reviewer, cv-review) all append a row.
