...
| `/cv-generator [--dry-run]`       | Generates a complete `cv-data.js` from `profile/*.md` files when cv-data.js is missing or corrupted. Extracts YAML frontmatter + markdown body, produces fully structured CV_DATA. Does not invent content.                       | —                                             |
...
| `cv-backup-agent` | Creates a versioned snapshot folder in `cv-versions/`: writes `cv-data.js` (with metadata header) and `locale-content.json` (11 locale content fields). Handles version conflict detection. | `/cv-backup`, `job-apply-orchestrator` |
